...
export async function getWorkoutEditorDataRepository(workoutId: string): Promise<WorkoutEditorData | null> {
    ...
    return {
        ...
        workoutExercises: (workoutExercisesResult.data as WorkoutEditorExercise[] | null) ?? [],
    }
}
...
export async function listAvailableExercisesRepository(workoutId: string): Promise<Exercise[]> {
    ...
    const usedExerciseIds = new Set((workoutExerciseResult.data ?? []).map((item) => item.exercise_id))

    return (exerciseResult.data ?? []).filter((exercise) => !usedExerciseIds.has(exercise.id))
}
