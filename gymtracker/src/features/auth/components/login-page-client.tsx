    ...
    return (
        <div className="min-h-dvh px-4 py-10 sm:px-6">
            <div className="mx-auto grid min-h-[calc(100dvh-5rem)] w-full max-w-6xl gap-8 lg:grid-cols-[1.05fr_0.95fr]">
                <Surface tone="accent" className="hidden overflow-hidden p-8 lg:flex lg:flex-col lg:justify-between">
                    <div>
                        ...
                        <div className="rounded-[1.75rem] border border-white/10 bg-white/8 p-5">
                            ...
                            <div className="mt-4 space-y-3">
                                {['Treino do dia com séries e cardio', 'Agenda semanal com remarcação', 'Histórico, calendário e evolução'].map((item) => (
                                    <div key={item} className="rounded-2xl border border-white/8 bg-black/10 px-4 py-3 text-sm text-zinc-100">
                                        {item}
                                    </div>
                                ))}
                            </div>
                ...
                <div className="flex items-center justify-center">
                    <Surface className="w-full max-w-md p-6 sm:p-8">
                        ...
                        <form onSubmit={handleLogin} className="space-y-5">
                            ...
                            <div>
                                <div className="mb-2 flex items-center justify-between gap-3">
                                    <FieldLabel htmlFor="password" className="mb-0">{t('password')}</FieldLabel>
                                    <button
                                        type="button"
                                        onClick={handlePasswordReset}
                                        disabled={resetting}
                                        className="rounded-lg px-1 text-xs font-semibold uppercase tracking-[0.16em] text-violet-400 transition-colors hover:text-violet-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-violet-300 disabled:opacity-60"
                                    >
                        ...
                        <div className="mt-6 space-y-2 text-center text-xs leading-6 text-zinc-500 dark:text-zinc-400">
                            <p>{t('footer')}</p>
                            <p>{t('contactHint')}</p>
                        </div>
                    </Surface>
